...
const addMonths = (date: Date, month: number) => {
  let dateClone = new Date(date.getTime())
...
const getArrayDateInRange = (startDate: Date, endDate: Date) => {
  let arrDay: any = [];
  let i = 0;
...
const getNumberDateInRange = (startDate: Date, endDate: Date) => {
  let i = 0;
...
const formatDate: any = (date: any, format: any = 'yyyy-MM-dd') => {
...
export { isLeapYear, getDaysInMonth, addDays, addMonths, getArrayDateInRange, getNumberDateInRange, formatDate };
